...
module.exports = class UPNPMediaServerRaumfeld extends UPNPMediaServer
{
    ...
    /**
     * creates a shuffle playlist
     * @param {String} container id where the shuffle should be generated from
     * @param {String} shuffle selection
     * @return {Promise} a promise with the playlist id and the playlist metadata
     */
    shuffle(_shuffleContainerId, _shuffleSelection)
    { 
        return this.callAction("ContentDirectory", "Shuffle", { "ContainerID" : _shuffleContainerId, "Selection" : _shuffleSelection });
    }


    /**
     * assigns a station button for a renderer
     * @param {String} the renderer id for what the station button shoul be set
     * @param {Integer} the station button
     * @param {String} the object id to store
     * @return {Promise} a promise with some result
     */
    assignStationButton(_rendererId, _buttonNum, _objectId)
    { 
        return this.callAction("ContentDirectory", "AssignStationButton", { "Renderer" : (_rendererId), "Button" : _buttonNum, "ObjectId" : _objectId});
    }
    
    /**
     * get the button assignement
     * @param {String} the renderer id for what the station button shoul be set
     * @param {Integer} the station button     
     * @return {Promise} a promise with some result
     */
    getStationButtonAssignment(_rendererId, _buttonNum)
    { 
        return this.callAction("ContentDirectory", "GetStationButtonAssignment", { "Renderer" : (_rendererId), "Button" : _buttonNum});
    }
