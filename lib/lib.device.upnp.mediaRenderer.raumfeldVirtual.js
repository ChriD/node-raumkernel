...
module.exports = class UPNPMediaRendererRaumfeldVirtual extends UPNPMediaRendererRaumfeld
{
     ...
     * @return {Promise} a promise with no result
     */
    loadContainer(_containerId, _trackNumber = 1)
    {
        var self = this;
        ...
        return new Promise(function(_resolve, _reject){
            try
            {
                 // generate the container uri for bending onto the media renderer
                var mediaServer = self.managerDisposer.deviceManager.getRaumfeldMediaServer(); 
                if(!mediaServer)
                {
                    self.logError("Calling loadContainer on renderer without a valid media server!");
                    _reject(new Error("Calling loadContainer on renderer without a valid media server!"));
                    return;
                }
                var uri = Tools.createAVTransportUriForContainer(mediaServer.udn(), _containerId, _trackNumber-1);
                self.bendAvTransportUri(uri, "").then(function(_data){
                        _resolve(_data)
                    }).catch(function(_data){
                        _reject(_data);
                    });
     ...
     * @return {Promise} a promise with no result
     */
    loadShuffle(_shuffleContainerId, _shuffleSelection)
    {
        // TODO: @@@
    }
