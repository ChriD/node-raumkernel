...
module.exports = class DeviceManager extends ManagerBase
{
    ...
    pingHost()
    {
        ...
        if(this.systemHost && this.systemHostFound)
        {
            ...
            Request({ url: "http://" + this.systemHost + ":" + this.getSettings().raumfeldHostRequestPort.toString() + "/ping", timout: 1500 }, function (error, response, body) {
                if(error || !response)
                {
