...
module.exports = class DeviceManager extends ManagerBase
{
    ...
    pingHost()
    {
        ...
        {
            this.logSilly("Check connection with the host!")
            Request({ url: "http://" + this.systemHost + ":" + "47365/ping", timout: 1500 }, function (error, response, body) {
                if(error || !response)
                {
        ...
        {
            this.logSilly("No system host found. Waiting for advertise!")
        }
    ...
    getVirtualMediaRenderer(_idOrChildName)
    {
        ...
        for(var udn of this.mediaRenderersVirtual.keys())
        {
            if(_idOrChildName == this.mediaRenderersVirtual.get(udn).name())
                return this.mediaRenderersVirtual.get(udn).name()
        }
        ...
        var mediaRenderer = this.getMediaRenderer();
        if(mediaRenderer)
        {
    ...
    getMediaRenderer(_idOrName)
    {
        ...
        // then check if perhaps the name of the renderer is the same
        for(var udn in this.mediaRenderers)
        {
            if(_idOrName == this.mediaRenderers[udn].name())
                return this.mediaRenderers[udn].name()
        }
        return null;
    }
