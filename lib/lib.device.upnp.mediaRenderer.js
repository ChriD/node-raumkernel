...
module.exports = class UPNPMediaRenderer extends UPNPDevice
{
    ...
    updateCurrentMediaItemInfo(_xmlMetadata)
    {
        ...
        mediaDataConverter.convertXMLToMediaList(_xmlMetadata).then(function(_data){
                // be sure the item is type of "audiotItem" and not album or something like that
                // Sometimes the AVTransportUriMetadata comes twice (Album and track) and current track metadata 
                // doesn't show up so we have to do this kind of fix 
                if(_data.length && _data[0].class.startsWith("object.item.audioItem"))
                {
    ...
    compareRendererTriggerValue(_triggerData)
    {
        var triggerDataObject = null
        var keyAlreadySet = false
        
        // get trigger data object
        if(_triggerData.roomUdn)                
            triggerDataObject = this.rendererState.rooms.get(_triggerData.roomUdn)                     
        else        
            triggerDataObject = this.rendererState        
                
        if(triggerDataObject)
        {
            if(_triggerData.values)
            {
                for(var i=0; i<_triggerData.values.length; i++)
                {             
                    if(triggerDataObject[_triggerData.key] && triggerDataObject[_triggerData.key].toString().toLowerCase() == _triggerData.values[i].toString().toLowerCase())
                        keyAlreadySet = true            
                }
            }
            else
            {
                if(triggerDataObject[_triggerData.key])
                    keyAlreadySet = triggerDataObject[_triggerData.key].toString().toLowerCase() == _triggerData.value.toString().toLowerCase()
            }
        }
        else
        {
            this.logError("No trigger data object found")
        }
    ...
    {    
        ...
        // be sure we have the renderer identificatrion in the trigger
        if(_triggerData && !_triggerData.rendererUdn)
            _triggerData.rendererUdn = this.udn()

        return new Promise(function(resolve, reject){
            var triggerCreated = false
            var keyAlreadySet = false;

            // check if one of the values is already set on the renderer, then we do not have to set up a trigger because
            // value is already there!
            keyAlreadySet = self.compareRendererTriggerValue(_triggerData)           
            if(!keyAlreadySet)                
            {
                ...
                self.managerDisposer.triggerManager.setupTrigger(_triggerContext, _triggerType, _triggerData, true, function(_data){ 
                    ...
                    // TODO: be sure it was not already rejected!
                    if(timeoutId)
